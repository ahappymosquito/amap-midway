...
set -e
echo "=== nginx site ==="
head -40 /etc/nginx/sites-available/amap.qrqto.club
echo "=== service ==="
...
PY
echo "=== https sni ==="
curl -sk --resolve amap.qrqto.club:443:127.0.0.1 -o /tmp/amap-cfg2.json -w "https:%{http_code}\n" https://amap.qrqto.club/api/config
python3 - <<'PY'
import json
